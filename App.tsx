...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm transition-colors duration-300">
      ...
      {/* Mobile Menu */}
      {isMenuOpen && (
        <div className="md:hidden absolute top-16 left-0 w-full bg-white dark:bg-slate-950 border-b border-slate-200 dark:border-slate-800 shadow-lg animate-in slide-in-from-top-5">
          <div className="flex flex-col p-4 space-y-4">
            <a 
              href="#about" 
              onClick={() => setIsMenuOpen(false)}
              className="text-slate-600 dark:text-slate-400 hover:text-navy dark:hover:text-blue-400 font-medium p-2 block hover:bg-slate-50 dark:hover:bg-slate-900 rounded-lg transition-colors"
            >
              About
            </a>
            <a 
...
const ImageLightbox: React.FC<{
  ...
  // Use a Portal so the lightbox renders directly on document.body,
  // bypassing any parent CSS transform stacking context.
  return ReactDOM.createPortal(
    <div
      className="fixed inset-0 z-[999] flex items-center justify-center p-4 sm:p-8"
      onClick={onClose}
      style={{ background: 'rgba(0,0,0,0.85)', backdropFilter: 'blur(8px)', animation: 'fadeIn 0.2s ease' }}
    >
      <style>{`
      ...
      <button
        onClick={onClose}
        className="absolute top-4 right-4 z-10 w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/25 text-white transition-all hover:scale-110 active:scale-95 backdrop-blur-sm"
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent px-6 py-4">
          ...
          <p className="text-white/60 text-xs mt-0.5">Press <kbd className="px-1.5 py-0.5 bg-white/20 rounded text-xs font-mono">ESC</kbd> or click outside to close</p>
        </div>
      </div>
    </div>,
    document.body
...
const ProjectCard: React.FC<{ project: typeof PROJECTS[0] }> = ({ project }) => {
  ...
  return (
  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 custom-shadow hover:-translate-y-2 transition-all group h-full flex flex-col overflow-hidden">
    {/* Project Image Preview */}
    {project.imageUrl && (
      <>
        <div
          className="relative w-full h-44 overflow-hidden bg-slate-100 dark:bg-slate-800 flex-shrink-0 cursor-zoom-in"
        ...
        >
          ...
          <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all duration-300 flex items-center justify-center">
            <div className="opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0 flex flex-col items-center gap-2">
              ...
              <span className="text-white text-[11px] font-semibold tracking-wide">Click to expand</span>
            </div>
          </div>
          <span className="absolute top-3 right-3 text-[10px] font-bold uppercase tracking-wider text-white bg-black/50 backdrop-blur-sm px-2.5 py-1 rounded-full">
            {project.role}
          </span>
        </div>
        {lightboxOpen && (
    ...
    <div className="p-7 flex flex-col flex-grow">
      {/* Header row (icon + role badge when no image) */}
      {!project.imageUrl && (
      ...
      {/* Title */}
      <h3 className="text-xl font-bold text-navy dark:text-slate-100 mb-3 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
        {project.title}
      </h3>
      ...
      {project.metrics && project.metrics.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-4">
          {project.metrics.map((metric, i) => (
            <span key={i} className="inline-flex items-center gap-1 text-[10px] font-bold px-2.5 py-1 rounded-full bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-100 dark:border-blue-800">
